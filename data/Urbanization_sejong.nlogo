...
patches-own[
  ...
  road1 ;;includes information on road types from 25 to 100
  ...
  slope
  excluded    ;;binary, 0=non-excluded, 100=excluded. The excluded image defines all locations that are resistant to urbanization.
  ...
  boundary ;;binary, 0=outside boundary, 1=within boundary
]
...
to setup
  ...
  ask patches with [road > 0][set run_value road1 / 100 * dispersion_coefficient]
...
to go
  ...
  ifelse show_roads [
    ...
    ask patches with [new_urbanized = 0 and urban = 0 and road = 0 and boundary = 1] [set pcolor grey]
    ask patches with [road > 0] [set pcolor black]]
  [
    ask patches with [new_urbanized = 1 and urban = 1] [set pcolor red]
    ask patches with [new_urbanized = 0 and urban = 0 and boundary = 1] [set pcolor grey]]
  tick
end
...
to load_data
  ...
  set urban-dataset gis:load-dataset "data/Urban_2018.asc"

  set slope-dataset gis:load-dataset "data/Slope_2014.asc"

  set road-dataset gis:load-dataset "data/Road_2018.asc"

  set excluded-dataset gis:load-dataset "data/Exclusion_2014.asc"

  ;;set landuse-dataset gis:load-dataset "data/landuse_santafe.asc"

  set boundary_dataset gis:load-dataset "data/Boundary.asc"

  gis:set-world-envelope gis:envelope-of urban-dataset

  gis:apply-raster urban-dataset urban
  ask patches [ifelse urban > 0 [set urban 1 set pcolor blue][set urban 0 set pcolor gray]  ] ;;for growth rules
...
to road_influenced_growth
  ask n-of tenpercent_urban (patches with [urban = 1])
    [let x random max_coefficient
      if  x < breed_coefficient
        [ road_seeking
          if road_found = 1 [
            ...
            while [i < run_value][
              ask road_pixel [
                set washere 1
                if count neighbors4 with [road > 0 and washere = 0] > 0 [
                  ask one-of neighbors4 with [road > 0 and washere = 0][
...
to road_seeking
  ...
  set road_found 0
  set road_pixel one-of patches with [road > 0] in-radius max_search
...
to export_data
  ...
  file-print "ncols         849   \r\n"
  file-print "nrows         1212   \r\n"
  file-print "xllcorner     211290.798000130308   \r\n"
  file-print "yllcorner     322863.241183900100   \r\n"
...
to-report tenpercent_urban
  report (round (count patches with [urban = 1] * 0.10))
end
@#$#@#$#@
GRAPHICS-WINDOW
358
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot (count patches with [urban = 1 and boundary = 1]) / (count patches with [boundary = 1]) * 100"
