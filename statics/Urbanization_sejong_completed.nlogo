...
globals [
  ...
  ;; Variables of our own definition
...
patches-own[
  ...
  road  ;; numerical, describes the road
  slope ;; numerical describes the percentage slope
  excluded    ;;binary, 0 not excluded, 100 excluded. The excluded image defines all locations that are resistant to urbanization.
  suitable   ;;binary, 0 if not suitable and 1 if it is suitable for urbanization
  boundary  ;; binary, 0 if outside boundary and 1 if within boundary
]
...
to setup
  ...
  ;; Visualisation of the data based on their properties
  ask patches [ifelse urban > 0  [set urban 1 set pcolor blue][set urban 0 set pcolor gray]  ]
  ask patches [if boundary = 0 [set pcolor black]]
  if show_roads [ask patches with [road > 0] [set pcolor black]]
end


to go
  ;; Calculate the dispersion value that we use in some of our growth rules
  set dispersion_value (dispersion_coefficient * 0.005) * (world-width ^ 2 + world-height ^ 2) ^ 0.5

  ;; Call upon the routines that perform the urban growth rules we've created
  spontaneous_growth
  new_spreading_center_growth
  ;;edge_growth

  road_growth ;; <<------ ADD YOUR PROCEDURE
  ...
  ifelse show_roads [
    ...
    ask patches with [road > 0] [set pcolor black]
  ]
  [
...
to load_data
  ...
  set urban-dataset gis:load-dataset "Urban_2018.asc"
  set slope-dataset gis:load-dataset "Slope_2014.asc"
  set road-dataset gis:load-dataset "Road_2018.asc"
  set excluded-dataset gis:load-dataset "Exclusion_2014.asc"
  set boundary_dataset gis:load-dataset "Boundary.asc"
  gis:set-world-envelope gis:envelope-of urban-dataset

  ;; Then apply the rasters onto the netlogo "world" with each square corresponding to a patch
  ...
  gis:apply-raster boundary_dataset boundary
end
...
to road_growth
  ask patches with [urban = 1][
    if any? neighbors with [road > 0][
      ask n-of 1 neighbors with [road > 0][
        ask neighbors4 with [road = 0 and urban = 0][
          if suitable = 1 [set urban 1 set new_urbanized 1]
        ]
      ]
    ]
...
to check_suitability
  ...
  ifelse x < item (slope) prob_to_build [set suitable 1][set suitable 0]
  if excluded = 100 [set suitable 0]
  if boundary = 0 [set suitable 0]
  ]
end
...
to show_slope
  ...
  ask patches [if (slope <= 0) or (slope >= 0) [set pcolor scale-color black slope min-slope max-slope]]
end
...
This is an instructive model on urban growth for the RM03 supervision 2 in 2021.
@#$#@#$#@
default
true
0
