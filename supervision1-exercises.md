...
1. Create new project: In the QGIS – Map view window, click `New Empty Project`.
2. Save your project: Click `Project` button in`Project Toolbar` and save as `.QGIS/.QGZ` to the working directory. 
3. Go to `Project` >  `Properties` in menu bar and open the `Project Properties` window. Check following tabs and edit: 
    - `General`: you can relocate your project file in the general settings, change the unit for distance measurement you prefer and also display coordinates units.
    - `Metadata`: It is suggested to inout title, author, creation date and a short abstract in the identification tab.
    - `CRS`: this tab provide coordinate reference system (CRS) setting for the project file. Be aware that CRS setting in the `Project Properties` is just for the project (called as `Data Frame setting` in ArcGIS). CRS setting for layers will be introduced later 
...
3: Layer ticked then can be found in layer panel. You can turn on and off layer by tick it or not. If you intend to process one layer, please make sure th target layer are ticked and selected with hightlight.
